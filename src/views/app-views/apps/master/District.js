...
import {getDistrict , updateDistrict, createDistrict, deleteDistrict } from 'utils/api/district';
...
const District = () => {
	...
	const onFinish = async (values) => {
		...
		if (isEdit && selectedId) {
			...
			const response = await updateDistrict(selectedId, name, short_code, mobile_no_ext, currency)
			if (response.success === true) {
				message.success(response.message)
			}
		} else {
			const response = await createDistrict(name, short_code, mobile_no_ext, currency);
			if (response.success === true) {
				message.success(response.message)
			}
		}
